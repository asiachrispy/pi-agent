# pi-agent 企业研发自动化工作区设计方案 v5.1

> **状态**：v5 经 `/team` 实测后的修订版  
> **日期**：2026-06-16  
...
v5.1 保留 v5 的核心方向：**一个 `/team` 命令驱动交付，一个 demand 追踪任务，一个 wiki 承载长期知识，一个 JTBD 管个人待办**。

本次根据 D-2026-002 到 D-2026-005 的实际运行情况做收敛：

- `/team` 的真实入口不是 `.pi/prompts/team.md`，而是 `.pi/extensions/team-entry/index.ts` 注册命令，协议由 `.pi/APPEND_SYSTEM.md` 与 `.pi/agents/team.md` 承载。
- `wiki/summary.md` 是全局第一跳快查表，`wiki/agent-reading-map.md` 只列增量必读文档，不在每一行重复 `summary.md`。
- demand frontmatter 必须能被脚本和 Pi.app 稳定解析：`status`、`step`、`weight`、`outcome` 使用短枚举；`owner` 必须明确；非阻塞任务不把 `team` 写成 `assignee`。
- 发布流程默认自动化打包、DMG、冷烟；只有 `push main`、`push tag`、`gh release create` 这类不可逆远端动作默认需要人工确认。
- `scripts/validate-demand.sh` 从格式检查升级为协议检查，用来防止重复 `workflow_update`、done 仍 pending、frontmatter 与 workflow_update 不一致等实际问题。
...
| 对象 | 用户理解 | 权威路径 |
|---|---|---|
| `/team` | 唯一日常入口：提需求、续跑、验证、发版 | `.pi/extensions/team-entry/index.ts` + `.pi/agents/team.md` |
...
pi-agent 的闭环不是“多角色聊天”，而是文件驱动的研发工作区：
...
`outcome` 不再写长总结。长总结放在 `## 交付`，便于 Pi.app 和脚本稳定解析。
...
- `workflow_update.status/step/weight` 与 frontmatter 不一致。
