...
### Changed
- **发布 pi-app v0.8.18 到 GitHub**：升版、重新 `package:macos`、制作 DMG、打 tag `v0.8.18` 并 `gh release create` 上传（详见下方条目与 pi-app CHANGELOG）。
- **重新部署 pi-app 到 199 / pi.gottao.com**（2026-07-14）：复用当日 standalone 构建（`b15a003` / 当时仍标 v0.8.17），打 `pi-app-runtime-livo-sso.tgz` 后走 `pi-app/.ops/deploy-pi-web-bundle-only.exp`（不改 systemd env）。release `pi-app-runtime-livo-sso-20260714141047`；本机 `PI_LOCAL_HEALTH:200`。公网验收：`/` 200、`/api/remote/client` 200、`/api/health` 403（loopback-only 预期）、`/api/sessions` 401、`/app/` 307 → Livo SSO start。
- **本地重新打包安装 Pi.app**（2026-07-14）：打包前同步上游——`pi` 合并 `upstream/main` 4 commits（`ed92c73c`，含 bedrock stop reasons / npm uninstall `--legacy-peer-deps` / encrypted_content backfill / anthropic empty usage；冲突保留 fork 的 `package-manager-npm` 拆分并把 legacy-peer-deps 落到该文件）并已 push；`pi-app` 确认相对 `agegr/pi-web` 落后 0。验证：`pi` `npm run check` 绿；`pi-app` `tsc --noEmit` + `vitest` 450/450 + `swift build` 绿后执行 `npm run package:macos`，`ditto` 安装到 `/Applications/Pi.app`（v0.8.17，standalone **238M**）。冷烟：内嵌 Node 跑 `server.js`，`/`、`/api/health`、`/api/sessions` 均 200。
- **拉取并合并 pi-web 上游（agegr/pi-web → pi-app）**（2026-07-14）：将 `pi-app` 的 `upstream` 远程从过期的 `asiachrispy/pi-web` 改回真正上游 `agegr/pi-web`，合并 `upstream/main`（v0.7.5–v0.7.11，`b3bcb4c`，落后 61 → 0）。主要合入：git worktree、@ 文件提及 / file-index、slash 命令面板与 queued messages、deferred thinking/media、plugins 配置、compaction 历史与 file-links、sidebar 运行态 SSE、session state reconciliation、工具 preset 抽到 `lib/tool-presets`（删除 `ToolPanel` / `sessions/new`）。冲突按归属原则保留 pi-app 鉴权、租户、macOS standalone、`@livos` 依赖与 AppShell/Sidebar/MessageView/FileViewer 的 i18n；`buildSessionContext` 保留全路径 UI 历史并将 compaction/branch_summary 映射为 `timelineSummary`。验证：`tsc --noEmit` 全绿、`vitest run` 450/450 通过。合并提交 `b15a003`。`ChatInput` i18n 已在 Unreleased 补回；`ChatWindow` 合并前本就无 i18n。
...
- **发布 pi @livos v0.80.6**（2026-07-13）：`pi` 打 tag `v0.80.6`（`2ebfe6fc`）并推送 `origin`；[`asiachrispy/pi` GitHub Release v0.80.6](https://github.com/asiachrispy/pi/releases/tag/v0.80.6) 已创建。**npm 发布待本机 `npm login` 后执行**：`cd pi && PI_NPM_SCOPE=@livos PI_NPM_PROVENANCE=0 node scripts/publish.mjs`（当前 registry token 401，`@livos/pi-*` 仍停留在 0.80.3）。
